...
def test_create_delete_project(connection):
    ...
    if str(project_name) in connection.get_all_projects():
        print('Successful project creation.' + ' All projects: ' +
              str(connection.get_all_projects()))
        project.delete_project()
        if not str(project_name) in connection.get_all_projects():
            print('Successful project deletion.' +
                  ' Remaining projects: ' + str(connection.get_all_projects()))
        else:
...
def test_retrieve_file(connection):
    file = connection.retrieve_file(
        "test3", "2022-07-23-13-03-33", "bebd60bc-7d92-4c9c-8939-c3aa866d0ae2.jpeg")
    print(file.id())


def test_retrieve_all_files_from_project_resource(connection):
    files = connection.retrieve_all_files_from_project_resource(
        "test3", "2022-07-23-13-03-33")
    print(files)


def test_remove_file_from_project(connection):
    file_id = connection.insert_file_into_project(
        project_name, file, "2022-07-23-13-03-33")
    print(connection.get_all_resources_from_project(project_name))
    print(len(connection.retrieve_all_files_from_project_resource(
        project_name, "2022-07-23-13-03-33")))
    connection.remove_file_from_project(
        project_name, "2022-07-23-13-03-33", file_id)
    print(len(connection.retrieve_all_files_from_project_resource(
        project_name, "2022-07-23-13-03-33")))
