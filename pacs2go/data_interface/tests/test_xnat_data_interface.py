...
class TestDataInterface(unittest.TestCase):
    ...
    def test_double_delete_project(self):
        ...
        p.delete_project()
        with self.assertRaisesRegex(Exception, f"Project {p.name} does not exist/has already been deleted."):
            p.delete_project()
    ...
    def test_get_directory_from_connection(self):
        ...
        directory = self.connection.get_directory(self.project.name, self.directory.name)
        directory_2 = XNATProject(self.connection,self.project.name).get_directory(self.directory.name)
